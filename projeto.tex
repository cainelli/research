...
\textbf{\large Descoberta e exploração de tópicos, documentos e comunidades em dados textuais com uso de Latent Dirichlet Allocation}\\
\vskip 4.5cm
\end{center}
...
No trabalho dos autores \citeonline{blei2003latent} foi proposto um algoritmo chamado Latent Dirichlet Allocation (LDA) \cite{blei2003latent} que possibilita descobrir tópicos intrinsecamente presentes em uma base de dados textual, a partir da sumarização de palavras relevantes. Em complemento ao LDA, os autores  \citeonline{sievert2014ldavis} propõem o LDAVis como uma proposta de visualizar os tópicos descobertos em uma base textual e as palavras que os descrevem. 
...
Métodos não probabilísticos como \textit{tf-idf} \cite{salton1983introduction}, por exemplo, utilizam contadores de palavras para descrever um documento.
 Por sua vez, o modelo LSI de \citeonline{Deerwester90indexingby} utiliza a decomposição em valores singulares (\textit{singular-value decomposition, SVD}) da
...
Por outro lado, modelos probabilísticos exploram uma outra abordagem de análise de documentos, caracterizando-se como uma categoria de variáveis latentes, a qual pode ser entendida como uma introdução ao conceito de tópicos. O pLSI, proposto por \citeonline{{hofmann1999probabilistic}}, 
